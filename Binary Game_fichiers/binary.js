...
Game.prototype = {
	...
	,swapBit: function(row,place,new_value) {
		...
		haxe_Log.trace(swaprow.toString(),{ fileName : "Game.hx", lineNumber : 50, className : "Game", methodName : "swapBit"});
		swaprow.bits[7 - place] = new_value;
		var currentsteps = swaprow.getStepsToRight();
		haxe_Log.trace(currentsteps.toString(),{ fileName : "Game.hx", lineNumber : 54, className : "Game", methodName : "swapBit"});
...
GameRow.intToString = function(num,base) {
	if(base == null) {
		base = 2;
	}
	var result_b = "";
	...
	while(i >= 0) {
		...
		result_b += Std.string("0123456789ABCDEFGHIJKLMNOPQRSTUVW".charAt(pow | 0));
		num -= pow * basePow | 0;
		--i;
	}
	var r = result_b;
...
GameRow.prototype = {
	...
	,checkCorrectInput: function() {
		haxe_Log.trace("chec for correct input",{ fileName : "GameRow.hx", lineNumber : 72, className : "GameRow", methodName : "checkCorrectInput", customParams : [GameRow.intToString(this.decimal,10),this.decimalString.join("")]});
	...
	,makeDecimal: function() {
		...
		haxe_Log.trace(this.toString(),{ fileName : "GameRow.hx", lineNumber : 169, className : "GameRow", methodName : "makeDecimal", customParams : [binstr]});
	}
	,toString: function() {
...
LevelDesc.prototype = {
	getNext: function() {
		...
		haxe_Log.trace(this.nextIndex,{ fileName : "LevelDesc.hx", lineNumber : 45, className : "LevelDesc", methodName : "getNext", customParams : [row.globalIndex]});
		return row;
	}
	,whatNext: function() {
		var row = new GameRow();
...
Main.main = function() {
	Main.main = Main;
	haxe_Log.trace("main started-------------------------",{ fileName : "Main.hx", lineNumber : 39, className : "Main", methodName : "main"});
	...
	var lolbtn = window.document.getElementById("lolMenuButton");
	...
	lolbtn.addEventListener("click",Main.clickedOnLol);
	...
	while(_g < 10) {
		...
		var curkey = ["" + n];
		haxe_Log.trace(curkey[0],{ fileName : "Main.hx", lineNumber : 99, className : "Main", methodName : "main"});
...
Main.clickedOnLol = function(e) {
	//var rowOn = LevelDesc.rows;
	//Main.stage.hideLayer(Main.getNext());
	//Main.stage.removeRow(0);
	Main.rowSolved(Main.game.currentlevel.rows[0]);
	//Main.stage.hideLayer("NumkeyboardLayer");
	//Main.game.rowRemoved(0);
...
Main.clearNumericValue = function(e) {
	...
	Main.numericInput.decimalString.pop();
	Main.updateNumeric();
};
...
Main.numkeyClick = function(key) {
	haxe_Log.trace("numkey clicked",{ fileName : "Main.hx", lineNumber : 147, className : "Main", methodName : "numkeyClick", customParams : [key]});
	Main.numericInput.decimalString.push(key);
	Main.numericInput.decimalString.shift();
	haxe_Log.trace(Main.numericInput.decimalString.toString(),{ fileName : "Main.hx", lineNumber : 165, className : "Main", methodName : "numkeyClick"});
	Main.updateNumeric();
};
Main.updateNumeric = function() {
	...
	TweenLite.from(Main.numericInput.decimal_display,0.3,{ css : { color : "#ffffff"}});
	TweenLite.fromTo("#numeKeyboardDisplay",0.4,{ alpha : 0},{ alpha : 1});
};
...
Main.resizeHandler = function(e) {
	...
	haxe_Log.trace("stage resized",{ fileName : "Main.hx", lineNumber : 272, className : "Main", methodName : "resizeHandler"});
};
Main.tick = function() {
	if(!Main.paused) {
		Main.game.tick();
...
Main.rowSolved = function(row) {
	//alert(Main.game.currentlevel.rows[0]);
	//alert(Main.game.currentlevel.nextIndex);
	Main.stage.removeRow(row);
};
...
Stage.prototype = {
	...
	,addNewRow: function(row) {
		...
		if(row.numeric) {
			...
			rowscontainer.addEventListener("click",function() {
				...
				haxe_Log.trace("click on",{ fileName : "Stage.hx", lineNumber : 276, className : "Stage", methodName : "addNewRow", customParams : [row]});
	...
	,numRowClick: function(r) {
		...
		var doTween = null;
		doTween = function() {
			TweenLite.from(r.decimal_display,0.5,{ css : { color : "#ff0000"}, onComplete : doTween});
		};
		doTween();
		Main.showInputFor(r);
	}
	...
	,showStartMenu: function() {
		...
		while(_g1 < _g) {
			var i = _g1++;
			TweenLite.from(letrs.item(i),1,{ y : -150, delay : 0.1 * i, ease : Bounce.easeOut});
		}
	}
	,showNewGame: function() {
		this.showLayer("gameLayer");
		this.updateLayout();
		TweenLite.to(this.binMarkLine,3,{ y : this.binffieldRect.height, onComplete : $bind(this,this.readyGo)});
		TweenLite.set(this.binBits,{ y : this.binffieldRect.height});
	}
	,readyGo: function() {
		Main.readyGo();
	}
	...
	,move: function(perc) {
		TweenLite.to([this.binBits,this.binMarkLine],1,{ y : this.binffieldRect.height * perc, ease : Power4.easeInOut});
	}
	,gameover: function() {
	...
	,removeRow: function(rowr) {
		...
		TweenLite.to(this.rowcontainers[row],0.5,{ alpha : 0.7, onComplete : $bind(this,this.addLife), onCompleteParams : [this.rowcontainers[row]]});
		var bitdivs = [];
		var _g = 0;
		while(_g < 8) {
			var n = _g++;
			...
			TweenLite.to(bitdiv,0.5,{ alpha : 0, rotation : Math.random() * 360, scaleX : 0, scaleY : 0, delay : n * 0.1});
		}
		this.rowcontainers[row].style.pointerEvents = "none";
		Main.rowRemoved(rowr);
	}
	,addLife: function(row) {
		TweenLite.to(row,1,{ css : { height : 0}, onComplete : $bind(this,this.removeCompl), onCompleteParams : [row], ease : Bounce.easeOut});
	}
	...
	,updateScore: function(currentscore) {
		...
		TweenLite.from(this.scorefield,0.5,{ css : { color : "#FFFFFF"}, scaleX : 1.2, scaleY : 1.2});
	}
...
StageBit.prototype = {
	...
	,clickList: function(e) {
		...
		TweenLite.to(this.bitdiv,0.1,{ scaleX : 0.5, scaleY : 1.1, onComplete : $bind(this,this.animationJoint), ease : Back.easeIn});
		return true;
	}
	,animationJoint: function() {
		this.swapBits();
		TweenLite.to(this.bitdiv,0.5,{ scaleX : 1, scaleY : 1, ease : Elastic.easeOut});
	}
	,swapBits: function() {
